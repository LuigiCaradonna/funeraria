...
watch(search, throttle(function(value) {
    Inertia.get('/sculptures', {search: value, field: field_to_order, order: list_order}, {
        preserveState: true,
        replace: true 
    });
}, 300));
...
let submitCU = () => {
    ...
    if ( new_entry )
        ...
        formCU.patch('sculptures/' + formCU.id, {
            onSuccess: () => {
                formCU.reset();
                document.getElementById('btnDismissModal').click();
            },
...
let submitD = () => {
    swal.fire({
        ...
        text: 'Stai per eliminare ' + formD.ids.length + ' sculture',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            formD.delete('/sculptures/multidel', {
            onSuccess: () => {
                formD.reset();
            },
        });
...
function deleteSculpture(a) {
    swal.fire({
        ...
        if (result.isConfirmed) {
            Inertia.delete('sculptures/' + a.id);
        }
    });
}
...
<template>
    ...
    <h1 class="text-center">Sculture</h1>
    <div class="d-flex justify-content-between">
        <div>
            <button @click="newSculpture" type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#sculptureModal">
            Nuova scultura
            </button>
    ...
    <div class="d-flex justify-content-around">
        <form @submit.prevent="submitD">
            <table class="table table-striped">
                ...
                <tbody>
                    <tr v-for="sculpture in sculptures.data" :key="sculpture.id">
                        <td><input type="checkbox" v-model="formD.ids" :value="sculpture.id" /></td>
                        <td>{{ sculpture.id }}</td>
                        <td>{{ sculpture.name }}</td>
                        <td>
                            <div class="d-flex justify-content-between">
                                <span role="button" data-bs-toggle="modal" data-bs-target="#sculptureModal" @click="editSculpture(sculpture)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16">
                                        <path d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708l-3-3zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207l6.5-6.5zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.499.499 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11l.178-.178z"/>
                                    </svg>
                                </span>
                                <span role="button" @click="deleteSculpture(sculpture)">
    ...
    <Pagination :links="sculptures.links" v-if="sculptures.links.length > 3"/>

    <div class="modal fade" id="sculptureModal" tabindex="-1" aria-labelledby="sculptureModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="sculptureModalLabel">Nuova scultura</h1>
                ...
                <div class="modal-body">
                    <form @submit.prevent="submitCU">
                        ...
                        <input type="hidden" v-model="formCU.id" id="sculptureId" />
                    </form>
                </div>
